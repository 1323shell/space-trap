...
const MainMenu = ({ classes, className = {} }) => {
  ...
  const [isSendLeaderboard, setIsSendLeaderboard] = useState(
  ...
  const saveUser = () => {
    localStorage.setItem(
      'userName',
      inputNameValue !== null ? inputNameValue : '',
    );
    localStorage.setItem(
      'userEmail',
      inputNameEmail !== null ? inputNameEmail : '',
    );
  ...
  return (
    <div className={classNames(classes.root, className.root)}>
      ...
      {!isUserEdit ? (
        ...
        >
          <div>
            <div className={classNames(classes.userInfo, className.userInfo)}>
              Player: {inputNameValue}
            </div>
            <div className={classNames(classes.userInfo, className.userInfo)}>
              Email: {inputNameEmail}
            </div>
          </div>
          <button onClick={() => setIsUserEdit(true)}>
        ...
        >
          ...
          <div>
            ...
            <button onClick={() => setIsUserEdit(false)}>
              <span role='img' aria-label='notSave'>
                ❌
              </span>
            </button>
          </div>
      ...
      <div
        className={classNames(
          classes.leaderboardActivate,
          className.leaderboardActivate,
        )}
      >
        <span>
          Do you want to get the <b>LEADERBOARD</b> in the end of the game?
        </span>
        <input
          type='checkbox'
          defaultChecked={isSendLeaderboard}
          className={classNames(
            classes.leaderboardCheckbox,
            className.leaderboardCheckbox,
          )}
