...
const update = (screenWidth, screenHeigth, timer, score, ship, asteroidArr, livesArr, fireArr, explArr) => {
    timer++;
    ...
    asteroidArr.forEach((asteroid, i) => {
        ...
        //borders
        if (asteroid.x >= screenWidth - 50 || asteroid.x <= 0) asteroid.speedX = -asteroid.speedX;
        if (asteroid) {
            if (asteroid.y >= screenHeigth + 50) asteroidArr.splice(i, 1);
        }

        //minus one live
        if (Math.abs(asteroid.x+25 - ship.x-25) < 50 && Math.abs(asteroid.y - ship.y) < 25) {
            livesArr.pop();
        ...
        for (let j = 0; j < fireArr.length; j++) {
            if (Math.abs(asteroid.x+25 - fireArr[j].x-15) < 50 && Math.abs(asteroid.y - fireArr[j].y) < 25) {
                explArr.push({
                    x: asteroid.x - 25,
                    y: asteroid.y - 25,
                    animX: 0,
